...
let rec exec (env:EnvironmentType) (exp:AST) :AST =
    ...
    //printpipe env
    match exp with
    | FuncDefExp(name,body,E) -> exec ((name,body)::env) E//(orderExec true env E)
    | Funcapp(Lambda(name,body),E) -> exec ((name,E)::env) body//(orderExec true env E)
